...
# Check tables
$PSQL -U "$dbuserId" collabsphere_users -c "\dt"              # users tables
$PSQL -U "$dbuserId" collabsphere_posts -c "\dt"              # posts tables
...
# Query data
$PSQL -U "$dbuserId" collabsphere_users -c "SELECT id, email, role FROM users LIMIT 10;"
$PSQL -U "$dbuserId" collabsphere_posts -c "SELECT count(*) FROM posts;"
...
# Connect with user
mongosh "mongodb://${mongoUserId}:${mongoUserPwd}@localhost:27017/admin"
...
# Check/create user
mongosh admin --eval "db.getUser('${mongoUserId}')"
```
...
curl -v http://localhost:8007/api/v1/users/auth/login \
  -H "Content-Type: application/json" \
  -d "{\"email\":\"admin@example.com\",\"password\":\"${SEED_DEFAULT_PASSWORD}\"}" 2>&1 | \
...
| `ddl-auto validate` or Flyway startup error | `tail -80 /tmp/<service>.log` | Fix/add a migration; don't switch back to `ddl-auto=update` |
